...
def analyze_content(text):
    ...
    keywords_str = ', '.join([k.capitalize() for k in top_keywords]) if top_keywords else 'N/A'
...
def process_content_for_display(content):
    """Convert YouTube URLs to embedded iframes in content"""
  ...
  <iframe src="https://www.youtube.com/embed/{vid_id}" 
    ...
    # Convert plain URLs (non-YouTube) to clickable links
...
def index():
    ...
    for post in posts:
        p = dict(post)
        p['display_content'] = process_content_for_display(p['content'])
...
def publish():
    ...
    return redirect(url_for('audit', title=title, keywords=keywords,
                            readability=readability, originality=originality,
...
def audit():
    ...
    return render_template('audit.html', title=title, keywords=keywords,
                           readability=readability, originality=originality,
                           word_count=word_count)
